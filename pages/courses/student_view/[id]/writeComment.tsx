...
const writeComment = () => {
    ...
    // use for animating success message
    ...
    const submitHandler = async (e: React.SyntheticEvent) => {
        ...
        const response = await fetch(`/api/anonymousComment`, {
            method: 'POST',
    ...
    useEffect(() => {
        if (notifyTransform >= 1.25) {
            ...
            setTimeout(() => {
                router.push(`/courses/student_view/${router.query.id}`)
            }, 1500);
        }
    }, [notifyTransform]);
        ...
        <>
            ...
            <div className="flex items-center justify-center h-screen bg-neutral-100">
                <div className="py-5 px-5 shadow-xl bg-white min-w-[50%] min-h-[50%]">
                    <h1 className="mt-5 mb-10 text-2xl text-slate-700">Write an Anonymous Comment</h1>
                    <div className="text-center">
                        <form onSubmit={submitHandler}>
                            <div>
                                <input value={message} onChange={onChange} placeholder="hello teacher, i think your course is ..." id="message" type="text" name="message" className="mb-5 placeholder-gray-300 w-[100%] text-slate-500 bg-gray-100 rounded-sm p-2 border-slate-200 border-2 focus:outline-none focus:border-neutral-100 focus:ring focus:ring-neutral-200" />
                            </div>
                            <div className="mt-2">
                                <button type="submit" className="bg-emerald-600 py-2 px-4 rounded-full mt-2 ml-30 shadow-lg text-neutral-100 drop-shadow focus:outline-none focus:bg-emerald-700 hover:bg-emerald-700 hover:scale-105">
                                    <p>SEND<FontAwesomeIcon icon={faPaperPlane} className="ml-2" /></p>
                                </button>
                            </div>
                        </form>
                    </div>
